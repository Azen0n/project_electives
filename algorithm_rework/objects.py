...
class Student:
    ...
    def get_elective_position(self, elective_id: int) -> int:
...
class Elective:
    ...
    def fill(self, students: list[Student]):
        ...
        for student in students:
            if self.id in student.priorities:
                ...
                if student.get_elective_position(self.id) == 1:
                    self.reserve += 1
    ...
    def sort_students_by_priority(self, smart_students_first: bool = True):
        ...
        if smart_students_first:
            self.students.sort(key=lambda student: (student.get_elective_position(self.id), 5 - student.performance))
        else:
            self.students.sort(key=lambda student: (student.get_elective_position(self.id), student.performance))
